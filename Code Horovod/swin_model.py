...
CFGS = {
    'swin_tiny_224': dict(input_size=(224, 224), window_size=7, embed_dim=96, depths=[1, 1, 1, 1], num_heads=[3, 6, 12, 24]),
...
# custom_cfg = {
#     'custom_swin': dict(input_size=(224, 224), window_size=7, embed_dim=96, depths=[1, 1, 1, 1], num_heads=[3, 6, 12, 24])
# }
...
class PatchEmbed(tf.keras.layers.Layer):
    def __init__(self, img_size=(224, 224), patch_size=(4, 4), in_chans=3, embed_dim=94, norm_layer=None):
...
class SwinTransformerModel(tf.keras.Model):
    def __init__(self, model_name='swin_tiny_patch4_window7_224', include_top=False,
                 img_size=(224, 224), patch_size=(4, 4), in_chans=3, num_classes=0,
                 embed_dim=94, depths=[1, 1, 1, 1], num_heads=[3, 6, 12, 24],
...
def SwinTransformer(model_name='swin_tiny_224', num_classes=0, include_top=True, pretrained=True, use_tpu=False, cfgs=CFGS):
    cfg = cfgs[model_name]
